...
public class QuickSort {
    ...
    /**
     * Recursively sorts the given array using the QuickSort algorithm.
     *
     * @param arr the array to be sorted
    ...
    private static void quickSortRecur(int[] arr, int left, int right) {
        while (left < right) {
            ...
            if (pivotIndex - left < right - pivotIndex) {
                quickSortRecur(arr, left, pivotIndex - 1);  // Sort the left partition
                left = pivotIndex + 1;  // Update the left pointer to sort the right partition
            } else {
                quickSortRecur(arr, pivotIndex + 1, right);  // Sort the right partition
